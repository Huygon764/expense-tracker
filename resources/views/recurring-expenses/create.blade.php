...
@section('page-title', __('messages.add_recurring'))

@section('content')
<div class="max-w-lg mx-auto">
    <x-card>
        <h1 class="font-display text-2xl font-bold text-on-surface mb-6">{{ __('messages.add_recurring') }}</h1>

        <form method="POST" action="{{ route('recurring-expenses.store') }}" class="space-y-5" id="recurring-form">
            @csrf

            <x-form-input name="title" :label="__('messages.title')" :value="old('title')" :required="true" maxlength="255" />
            ...
            <x-form-select name="type" :label="__('messages.frequency')" :required="true">
                <option value="weekly" {{ old('type', 'monthly') === 'weekly' ? 'selected' : '' }}>{{ __('messages.weekly') }}</option>
            ...
            <div id="day-of-week-wrap" style="display: {{ old('type', 'monthly') === 'weekly' ? 'block' : 'none' }};">
                <x-form-select name="day_of_week" :label="__('messages.day_of_week')">
                    <option value="0" {{ old('day_of_week') === '0' ? 'selected' : '' }}>{{ __('messages.sunday') }}</option>
                    <option value="1" {{ old('day_of_week') === '1' ? 'selected' : '' }}>{{ __('messages.monday') }}</option>
                    <option value="2" {{ old('day_of_week') === '2' ? 'selected' : '' }}>{{ __('messages.tuesday') }}</option>
                    <option value="3" {{ old('day_of_week') === '3' ? 'selected' : '' }}>{{ __('messages.wednesday') }}</option>
                    <option value="4" {{ old('day_of_week') === '4' ? 'selected' : '' }}>{{ __('messages.thursday') }}</option>
                    <option value="5" {{ old('day_of_week') === '5' ? 'selected' : '' }}>{{ __('messages.friday') }}</option>
                    <option value="6" {{ old('day_of_week') === '6' ? 'selected' : '' }}>{{ __('messages.saturday') }}</option>
                </x-form-select>
            </div>
