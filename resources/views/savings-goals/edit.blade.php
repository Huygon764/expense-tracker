...
@section('content')
<div class="max-w-md">
    <h1 class="text-2xl font-semibold text-gray-900 dark:text-gray-100 mb-6">Edit savings goal</h1>
    ...
    <form method="POST" action="{{ route('savings-goals.update', $savingsGoal) }}" class="space-y-4">
        ...
        <div>
            ...
            <input type="text" name="name" id="name" value="{{ old('name', $savingsGoal->name) }}" required maxlength="255"
        ...
        <div>
            ...
            <input type="number" name="target_amount" id="target_amount" value="{{ old('target_amount', $savingsGoal->target_amount) }}" required min="0" step="0.01"
                class="mt-1 block w-full rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-3 py-2 text-gray-900 dark:text-gray-100">
            @error('target_amount')
                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
            @enderror
        ...
        <div>
            ...
            <input type="date" name="deadline" id="deadline" value="{{ old('deadline', $savingsGoal->deadline->format('Y-m-d')) }}" required
                class="mt-1 block w-full rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-3 py-2 text-gray-900 dark:text-gray-100">
            @error('deadline')
                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
            @enderror
