...
@section('title', __('messages.add_budget'))

@section('content')
<div class="max-w-md">
    <h1 class="text-2xl font-semibold text-gray-900 dark:text-gray-100 mb-6">{{ __('messages.add_budget') }}</h1>
    ...
    <form method="POST" action="{{ route('budgets.store') }}" class="space-y-4">
        ...
        <div class="flex gap-3 pt-2">
            <button type="submit" class="rounded-md bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-500">{{ __('messages.add_budget') }}</button>
